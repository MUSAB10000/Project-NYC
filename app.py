# app.py
# NYC Airbnb Dashboard — Plotly MapLibre (no Mapbox), memory-safe maps, KPIs & sample, borough-based prediction
...
# ---- numeric coercion + downcast to reduce memory footprint
for col in [
...
intish_cols = ["minimum_nights", "number_of_reviews", "calculated_host_listings_count", "availability_365"]
df[float_cols] = df[float_cols].astype("float32")
for c in intish_cols:
    if c in df:
...
# Performance toggle
speed_mode = st.sidebar.toggle("⚡ Speed mode (recommended)", value=True,
...
# ===============================================
# 1) Overview — KPIs + Sample + Describe
# ===============================================
if tab == "Overview":
    st.header("📊 Dataset Overview")
    k1, k2, k3, k4 = st.columns(4)
    with k1: st.metric("Avg Price ($)", f"{float(df_filtered['price'].mean()):.2f}")
    with k2: st.metric("Avg Minimum Nights", f"{float(df_filtered['minimum_nights'].mean()):.1f}")
    with k3: st.metric("Avg Reviews/Month", f"{float(df_filtered['reviews_per_month'].fillna(0).mean()):.2f}")
    with k4: st.metric("Avg Host Listings", f"{float(df_filtered['calculated_host_listings_count'].mean()):.1f}")
    ...
    borough_counts = (
        df_filtered["neighbourhood_group"].value_counts()
        .rename_axis("Borough").reset_index(name="Count")
    ...
    room_counts = (
        df_filtered["room_type"].value_counts()
        .rename_axis("Room Type").reset_index(name="Count")
        .sort_values("Count", ascending=False)
    )
    fig = px.bar(room_counts, x="Room Type", y="Count",
                 color="Room Type", text="Count",
...
# ===============================================
    ...
    fig = px.bar(avg_price_borough, x="neighbourhood_group", y="price",
                 color="neighbourhood_group", text="price",
    ...
    fig = px.bar(avg_price_room, x="room_type", y="price",
                 color="room_type", text="price",
    ...
    st.plotly_chart(
        px.scatter(df_filtered, x="minimum_nights", y="price",
                   color="neighbourhood_group", log_y=True,
    ...
    st.plotly_chart(
        px.scatter(df_filtered, x="availability_365", y="price",
                   color="room_type", log_y=True,
    ...
    avg_min_nights_borough = (
        ...
        .mean().sort_values(by="minimum_nights", ascending=False)
    )
    fig = px.bar(avg_min_nights_borough, x="neighbourhood_group", y="minimum_nights",
                 color="neighbourhood_group", text="minimum_nights",
    ...
    st.plotly_chart(
        px.imshow(pivot_table, labels=dict(x="Room Type", y="Borough", color="Mean Price"),
                  x=pivot_table.columns, y=pivot_table.index, color_continuous_scale="YlGnBu",
    ...
    st.plotly_chart(
        px.histogram(df_filtered, x="price", nbins=50, color="room_type",
                     title="Price Distribution by Room Type", marginal="box", log_y=True),
        use_container_width=True
    )

    st.subheader("8. Top 20 Most Expensive Listings")
    top_expensive = df_filtered.nlargest(20, "price")
    fig = px.bar(top_expensive.sort_values(by="price", ascending=False),
                 x="name", y="price", color="neighbourhood_group", text='price',
...
# ===============================================
# 3) Geospatial Analysis — 8 MAPS (MapLibre APIs)
# ===============================================
elif tab == "Geospatial Analysis":
    st.header("🌍 Geospatial Analysis (MapLibre)")

    # Clean geo rows & keep plausible NYC bounds
    geo_df = df_filtered.dropna(subset=["latitude", "longitude"]).copy()
    geo_df = geo_df[
        geo_df["latitude"].between(40.4, 41.1)
        & geo_df["longitude"].between(-74.3, -73.6)
    ]
    if geo_df.empty:
        st.warning("No valid latitude/longitude points after filtering. Try widening filters.")
        st.stop()

    # Performance: smaller samples avoid MemoryError during Plotly JSON serialization
    MAX_POINTS = 5000 if speed_mode else 12000
    df_sample = geo_df.sample(min(MAX_POINTS, len(geo_df)), random_state=42).copy()

    # Keep hover compact to save memory
    ...
    # 3) Room Type Colored Map (symbol-free for compatibility)
    ...
    # 5) Neighborhood Mean-Price Bubble Map (size = listing_count, color = avg_price)
...
# ===============================================
    ...
    st.plotly_chart(
        px.histogram(df_filtered, x="number_of_reviews", nbins=50, color="neighbourhood_group",
                     title="Number of Reviews Distribution by Borough", log_y=True),
        use_container_width=True
    )

    st.subheader("2. Reviews per Month Distribution")
    st.plotly_chart(
        px.histogram(df_filtered, x="reviews_per_month", nbins=50, color="room_type",
                     title="Reviews per Month Distribution by Room Type", log_y=True),
        use_container_width=True
    )

    st.subheader("3. Host Listings Count Distribution")
    st.plotly_chart(
        px.histogram(df_filtered, x="calculated_host_listings_count", nbins=30, color="neighbourhood_group",
                     title="Host Listings Count Distribution by Borough", log_y=True),
        use_container_width=True
    )
    ...
    avg_reviews_borough = (
        ...
        .mean().fillna(0).sort_values(by="reviews_per_month", ascending=False)
    )
    fig = px.bar(avg_reviews_borough, x="neighbourhood_group", y="reviews_per_month",
                 color="neighbourhood_group", text="reviews_per_month",
    ...
    avg_host_listings_room = (
        ...
        .mean().sort_values(by="calculated_host_listings_count", ascending=False)
    )
    fig = px.bar(avg_host_listings_room, x="room_type", y="calculated_host_listings_count",
                 color="room_type", text="calculated_host_listings_count",
    ...
    top_hosts = (
        df_filtered.groupby("host_id").agg(
            total_listings=("id", "count"),
            avg_price=("price", "mean")
        ).reset_index().sort_values("total_listings", ascending=False).head(20)
    )
    top_hosts["host_label"] = top_hosts["host_id"].astype(str)
    fig = px.bar(top_hosts, x="host_label", y="total_listings",
                 color="host_label", title="Top 20 Hosts by Listings",
    ...
    st.plotly_chart(
        px.scatter(df_filtered.dropna(subset=["reviews_per_month", "price"]),
    ...
    st.plotly_chart(
        px.scatter(df_filtered.dropna(subset=["reviews_per_month", "availability_365"]),
                   x="reviews_per_month", y="availability_365",
                   color="room_type",
    ...
    st.plotly_chart(px.pie(room_share, names="room_type", values="count", title="Room Type Share"),
                    use_container_width=True)

# ===============================================
# 5) Prediction — Borough-picked
# ===============================================
elif tab == "Prediction":
    ...
    if st.button("Predict Price"):
        ...
        try:
            prediction = float(xgb_model.predict(input_data)[0])
            prediction = max(0.0, prediction)
            st.success(f"💰 Predicted Nightly Price in {pred_borough}: ${prediction:.2f}")
        except Exception as e:
